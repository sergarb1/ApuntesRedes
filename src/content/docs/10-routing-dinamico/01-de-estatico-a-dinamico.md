...
> 🗺️ **Estás en:** 🗣️ **U10 · Routing dinámico OSPF** → 01 · De estático a dinámico
...
En la [U09 · Routing y ACLs](/ApuntesRedes/09-routing-acls) escribiste rutas con `ip route` una a una. Funcionan, pero imagina que la empresa crece: 50 routers, 200 redes, y cada vez que cae un enlace tienes que saltar a la consola a corregir rutas. Ese es el problema que resuelve este punto.
...
**Analogía del GPS:** una ruta estática es como marcar en un mapa la ruta a mano y pegarla en el salpicadero: si cortan una carretera, sigues yendo a la calle cortada. El routing dinámico es el GPS en modo "recalcular": detecta el atasco y te reencamina al momento. Precisamente esa "recalculación" automática se llama **convergencia**, y la estudiarás con la comparativa del [punto 2](/ApuntesRedes/10-routing-dinamico/02-igp-vs-egp).
...
- **Ruta de respaldo (`floating static`):** la ruta principal la aprende el protocolo dinámico (p. ej. OSPF, AD 110); el respaldo es una estática con *administrative distance* más alta, que solo se activa si la dinámica desaparece:
  ```bash
  ; Ruta principal: 0.0.0.0/0 aprendida por OSPF (AD 110, ver [punto 8](/ApuntesRedes/10-routing-dinamico/08-ruta-por-defecto-y-diagnostico))
...
📚 [Volver al índice de la unidad](/ApuntesRedes/10-routing-dinamico) · **Anterior:** [Índice de la unidad](/ApuntesRedes/10-routing-dinamico) · **Siguiente:** [02 · IGP vs EGP y RIP vs OSPF](/ApuntesRedes/10-routing-dinamico/02-igp-vs-egp)